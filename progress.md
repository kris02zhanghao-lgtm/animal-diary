...
- Supabase Free Plan 项目会因连续低活跃被自动暂停；本项目已遇到过暂停后首屏无法建立匿名会话的问题
- 面试展示期间至少每 7 天做一次真实线上访问，建议每 3-5 天打开生产地址并触发一次记录读取或保存/删除测试记录，确保 Supabase 产生数据库/API 活动
- 面试前一天和当天早上手动进入 Supabase Dashboard 确认 `animal-diary` 项目状态为 Active；如显示 Paused，先 Resume，等待恢复后再打开线上地址验证
- 不要通过清理浏览器 localStorage 来处理会话问题；匿名身份与历史记录访问绑定，清理后可能看不到原设备记录
