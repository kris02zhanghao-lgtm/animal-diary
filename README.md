...
- 本项目当前为移动端优先设计的 Web App
- 如果在电脑端打开，建议使用浏览器开发者工具切换到手机尺寸体验
- 由于比赛周期有限，本次版本优先完成了移动端核心功能闭环，桌面端完整适配会作为后续迭代继续完善

## 快速试用

- 仓库根目录的 `sample-animal-images/` 提供了 16 张可直接上传的动物示例图片
- 如果你是评审或第一次体验，打开线上地址后可直接使用该目录中的图片测试识别、编辑、保存、图鉴和报告流程
- 当前目录包含猫、狗、松鼠、鸽子、刺猬、小熊猫等不同动物，便于快速体验识别效果和图鉴聚合效果

## 线上地址

> 当前线上版本部署在 Vercel，国内网络环境下访问可能不稳定

https://animal-diary.vercel.app/
## 体验建议
